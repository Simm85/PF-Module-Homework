function manOWar(input) {
    ...
    for (const line of input) {
        ...
        switch (command) {
            case 'Fire': {
                let [index, damage] = values;
                if (index >= 0 && index < warShip.length) warShip[index] -= damage;
                if (warShip[index] <= 0)
                    return console.log("You won! The enemy ship has sunken.");
            } break;

            case 'Defend':
                let [startIndex, endIndex, damage] = values;
                const isValid = (startIndex >= 0 && startIndex < pirateShip.length) && (endIndex >= 0 && endIndex < pirateShip.length);
                if (isValid) {
                    while (startIndex <= endIndex) {
                        pirateShip[startIndex] -= damage;
                        if (pirateShip[startIndex] <= 0) return console.log("You lost! The pirate ship has sunken.");
                        startIndex++;
                    }
                }
                break;
            ...
            case 'Status':
                const percent = 0.2 * maxHealth;
                let forRepair = 0;
                for (let i = 0; i < pirateShip.length; i++) {
                    if (pirateShip[i] < percent) forRepair++;
                }
            ...
            case 'Retire':
                let sum = 0
                pirateShip.forEach(section => sum += section);
                console.log(`Pirate ship status: ${sum}`);
                sum = 0;
                warShip.forEach(section => sum += section);
                console.log(`Warship status: ${sum}`);
                break;
        }
    }
}
