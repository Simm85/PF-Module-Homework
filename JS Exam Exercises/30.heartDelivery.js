function heartDelivery(input) {
    let neighborhoods = input.shift().split('@').map(n => Number(n));
    let negativeCount = 0;
    let lastIndex = 0;
    let moreThanOneVisit = false;
    for (let line of input) {
        let [command, jumpIndex] = line.split(' ');
        jumpIndex = Number(jumpIndex);
        if (command === 'Love!') return printResults(lastIndex);
        if (moreThanOneVisit) jumpIndex += lastIndex;
        if (jumpIndex >= neighborhoods.length) jumpIndex = 0;
        if (neighborhoods[jumpIndex] > 0) {
            neighborhoods[jumpIndex] -= 2;
            moreThanOneVisit = true;
            if (neighborhoods[jumpIndex] === 0) console.log(`Place ${jumpIndex} has Valentine's day.`);
        } else {
    ...
    function printResults(jumpIndex) {
        console.log(`Cupid's last position was ${jumpIndex}.`);
        let successful = true;
        for (let neighbor of neighborhoods) {
            if (neighbor > 0) {
                negativeCount++;
                successful = false;
            }
        ...
        successful
            ...
            : console.log(`Cupid has failed ${negativeCount} places.`);
    }
}
